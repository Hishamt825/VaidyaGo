...
walkSync(srcDir, (filePath) => {
    if (filePath.endsWith('.jsx') || filePath.endsWith('.js') || filePath.endsWith('.css')) {
        ...
        // Make sure we don't accidentally replace already updated ones: src="/assets/image.png"
        // regex explanation: look for src=" or src=' followed by optional /, then filenames that NOT start with assets/
        const srcRegex = /(src|href|img)=["']\/?(?!(?:assets\/))([^/"']+\.(png|jpg|jpeg|svg|gif|webp))["']/gi;
        content = content.replace(srcRegex, '$1="/assets/$2"');

        // Replace CSS url('/image.png') -> url('/assets/image.png')
