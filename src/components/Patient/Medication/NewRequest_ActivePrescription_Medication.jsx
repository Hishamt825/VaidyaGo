...
const NewRequest_ActivePrescription_Medication = ({ onClose, onRequestAdded }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            delivery_preference: deliveryPreference === 'home' ? 'home_delivery' : 'pickup'
        };

        try {
            // Updated endpoint based on Postman screenshot
            const response = await apiFetch(`${BASE_URL}/api/prescriptions/prescription-request/add/`, {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                const errorMsg = data.detail || (data.medication ? data.medication[0] : null) || (data.pharmacy ? data.pharmacy[0] : null) || 'Failed to submit request.';
                setMessage(errorMsg);
            }
        } catch (error) {
                    ...
                    <div className="flex justify-between items-start mb-6">
                        ...
                        <div className="w-[42px] h-[42px] bg-[#DFEEF0] rounded-full flex items-center justify-center text-[#1A7785]">
                            ...
                            <svg className="w-[22px] h-[22px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.2" d="M9 5H15M9 5V3H15V5M9 5H5V21H19V5H15M8 11H16M8 15H13" />
                            </svg>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button 
                                    ...
                                    className={`rounded-[16px] p-4 flex flex-col items-center gap-1.5 transition-all ${deliveryPreference === 'pickup' ? 'bg-[#EAEFF2] border-[1.5px] border-[#1A7785]' : 'bg-[#EAEFF2] border-[1.5px] border-transparent opacity-60 hover:opacity-100'}`}
                                >
                                    <svg className={`w-5 h-5 ${deliveryPreference === 'pickup' ? 'text-[#1A7785]' : 'text-[#627382]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <span className={`text-[14px] font-bold ${deliveryPreference === 'pickup' ? 'text-[#0D1C2E]' : 'text-[#627382]'}`}>Pickup</span>
                                </button>
                                <button 
                                    type="button"
                                    onClick={() => setDeliveryPreference('home')}
                                    className={`rounded-[16px] p-4 flex flex-col items-center gap-1.5 transition-all ${deliveryPreference === 'home' ? 'bg-[#EAEFF2] border-[1.5px] border-[#1A7785]' : 'bg-[#EAEFF2] border-[1.5px] border-transparent opacity-60 hover:opacity-100'}`}
                                >
                                    <svg className={`w-5 h-5 ${deliveryPreference === 'home' ? 'text-[#1A7785]' : 'text-[#627382]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <span className={`text-[14px] font-bold ${deliveryPreference === 'home' ? 'text-[#0D1C2E]' : 'text-[#627382]'}`}>Home Delivery</span>
                                </button>
                            </div>
                        </div>
                    </div>
...
export default NewRequest_ActivePrescription_Medication;
